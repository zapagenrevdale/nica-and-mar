const Entourage: React.FC<Props> = ({ title, names }) => {
  return (
    <div className="text-center flex flex-col gap-3  items-center font-woodland ">
      <div>
        <h3 className="text-base sm:text-lg lg:text-xl my-1 px-1  md:bg-none rounded-lg font-bold">{title}</h3>
      ...
      <div className="text-xs md:text-base lg:leading-8 tracking-wider">
        {names.map((name) => (
          <p key={name} className="mt-2 px-1  md:bg-none rounded-lg">{name}</p>
        ))}
      </div>
    </div>
  );
